...
import { updateMerchant } from 'services/manufcature'
import Form from './Form'

const URL = CATALOG_API_URL.manufacture
...
const CountryEditFormIndex = (props) => {
  ...
  const submitData = async (url, values, method = 'PATCH') => {
    const res = await updateMerchant(url, values, method)
    if (res?.success) {
  ...
  const title = id ? 'Edit Manufacture' : 'Add Manufacture'
  return (
    <div>
      <Helmet title={title} />
      <div className="card">
