...
const FormA = ({ initialValues, onSuccess }) => {
  ...
  const fetchSubmit = async (value) => {
    const a = initialValues
      ? await editData(`${CATALOG_API_URL.saltsynonims}/${initialValues._id}`, value, 'json')
      : await editData(`${CATALOG_API_URL.saltsynonims}/create`, value, 'json', 'POST')
    setSubmitting(false)
    if (!a.error) {
      onSuccess()
  ...
  const formItems = [
    ...
    {
      type: <AddNew pullRight={false} add onClick={handleAdd} attribute="Salt Name" />,
      key: 'add test',
