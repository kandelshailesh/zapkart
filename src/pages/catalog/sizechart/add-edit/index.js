...
const URL = CATALOG_API_URL.sizeChart
...
const CountryEditFormIndex = (props) => {
  ...
  if (id) {
    form = (
      <Query url={`${URL}/query?chartId=${id}`} loader={<Skeleton active />}>
        {(res) => {
          if (res?.data[0]) return <Form initialValues={res.data[0]} onSubmit={onEdit} />
          return <div>No data!</div>
        }}
      </Query>
    )
  }
  const title = id ? 'Edit Sizechart' : 'Add Sizechart'
  return (
    <div>
      <Helmet title={title} />
      <div className="card">
